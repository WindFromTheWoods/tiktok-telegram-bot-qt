/******************************************************************************
 * @file    AppConfig.cpp
 * @brief   Implements validation and construction of runtime bot configuration.
 *
...
AppConfig::AppConfig(QString botToken, QString channelId, const qint64 adminUserId,
                     QString ytDlpExecutable, const std::chrono::minutes publicationInterval)
...
std::optional<AppConfig> AppConfig::fromEnvironment(QString* error)
{
    return fromValues(environmentValue("TELEGRAM_BOT_TOKEN"),
                      ...
                      environmentValue("TELEGRAM_ADMIN_USER_ID"), environmentValue("YTDLP_PATH"),
...
std::optional<AppConfig> AppConfig::fromValues(QString botToken, QString channelId,
                                               QString adminUserId, QString ytDlpExecutable,
                                               QString publicationIntervalText, QString* error)
{
    ...
    if (!publicationIntervalText.isEmpty())
    {
        ...
        {
            problems.append(
                QStringLiteral("TELEGRAM_POST_INTERVAL_MINUTES must be between 1 and 10080"));
        }
        else
        {
    ...
    return AppConfig(std::move(botToken), std::move(channelId), parsedAdminUserId,
                     std::move(ytDlpExecutable), std::chrono::minutes(publicationIntervalMinutes));
}
