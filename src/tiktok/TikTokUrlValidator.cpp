...
{
    static const QRegularExpression shortCodePattern(
...
{
    if (!url.isValid() || url.isRelative() || url.scheme().compare(QStringLiteral("https"),
                                                                   Qt::CaseInsensitive)
            != 0
    ...
    const QStringList segments = url.path(QUrl::FullyDecoded)
