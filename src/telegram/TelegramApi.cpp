...
void TelegramApi::sendVideo(const QString &chatId, const QString &filePath,
...
TelegramApiResponse TelegramApi::parseResponse(const QByteArray &payload,
                                               ...
                                               const QString &networkError)
{
    ...
    if (parseError.error != QJsonParseError::NoError || !document.isObject()) {
        response.description = networkError.isEmpty()
            ? QStringLiteral("Telegram returned malformed JSON: %1").arg(parseError.errorString())
    ...
    response.errorCode = object.value(QStringLiteral("error_code")).toInt(httpStatusCode);
    ...
    if (!networkError.isEmpty() && response.ok) {
        response.ok = false;
        response.description = QStringLiteral("Telegram network request failed: %1").arg(networkError);
    }
    if (!response.ok && response.description.isEmpty()) {
        if (!networkError.isEmpty()) {
            response.description = QStringLiteral("Telegram network request failed: %1")
                                       .arg(networkError);
        } else if (httpStatusCode > 0) {
            response.description = QStringLiteral("Telegram request failed with HTTP status %1")
                                       .arg(httpStatusCode);
        } else {
...
QNetworkRequest TelegramApi::makeRequest(const QString &method) const
{
    ...
    request.setHeader(QNetworkRequest::UserAgentHeader,
                      QStringLiteral("TikTokTelegramBot/1.0"));
